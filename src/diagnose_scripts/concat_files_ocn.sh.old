...
if [ ! -f flag_notrans_ocn ] ; then
    for y in $( seq $diag_beg_year $diag_end_year ); do
        for m in $( seq 1 12 ); do
            old_file=${casename}.EMOM.h0.mon.$( printf "%04d-%02d" $y $m ).nc
            new_file_bilinear=$( echo "$old_file" | sed -e 's/\.EMOM\./.EMOM_rg./' )
            new_file_conserve2nd=$( echo "$old_file" | sed -e 's/\.EMOM\./.EMOM_rg2./' )
    ...
    if [ "$old_files" != "" ] ; then
        ...
        julia $script_coordtrans_dir/transform_data.jl --s-file=$old_files --d-file=$new_files_bilinear --w-file=${wgt_dir}/wgt.bilinear.nc --algo=ESMF --vars=HMXL --x-dim=Nx --y-dim=Ny --z-dim=Nz --t-dim=time --s-dir=$ocn_hist_dir --d-dir=$concat_dir
