...
let
    ...
    _atm_heat_conv = - ( FFLX_TOA - FFLX_SFC - HFLX_SFC - PRECS * ρ_fw * L_fusion )  # ~ - ∂T/∂t
    _ocn_heat_conv = - ( FFLX_SFC + HFLX_SFC + PRECS * ρ_fw * L_fusion )

    Nt = end_t - beg_t + 1

    global ATM_TFLX_CONV = zeros(Float64, length(r.lat_bnd)-1, Nt)
    global OCN_TFLX_CONV = zeros(Float64, length(r.lat_bnd)-1, Nt)
    global AHT       = zeros(Float64, length(r.lat_bnd), Nt)
    global OHT       = zeros(Float64, length(r.lat_bnd), Nt)

    for t = 1:Nt
        atm_v = view(_atm_heat_conv, :, :, t)
        ATM_TFLX_CONV[:, t] = MapTransform.transform(r, atm_v) 
        AHT[:, t]       = MapTransform.∫∂a(r, atm_v)
        
        ocn_v = view(_ocn_heat_conv, :, :, t)
        OCN_TFLX_CONV[:, t] = MapTransform.transform(r, ocn_v) 
        OHT[:, t]       = MapTransform.∫∂a(r, ocn_v)
    end
...
Dataset(parsed["output-file"], "c") do ds
    ...
    for (varname, vardata, vardim, attrib) in [
        ("ATM_TFLX_CONV",   ATM_TFLX_CONV,                           ("lat", "time"), Dict()),
        ("AHT",        AHT,                                  ("lat_bnd", "time"), Dict()),
        ("ATM_TFLX_CONV_MEAN",   mean(ATM_TFLX_CONV, dims=2)[:, 1],  ("lat",), Dict()),
        ("AHT_MEAN",         mean(AHT, dims=2)[:, 1],        ("lat_bnd",), Dict()),
        ("AHT_AM",           AHT_AM,                         ("lat_bnd", "year",), Dict()),
#        ("ITCZ_lat",         ITCZ_lat,                       ("year",), Dict()),
 
        ("OCN_TFLX_CONV",      OCN_TFLX_CONV,                     ("lat", "time"), Dict()),
        ("OHT",                OHT,                               ("lat_bnd", "time"), Dict()),
        ("OCN_TFLX_CONV_MEAN", mean(OCN_TFLX_CONV, dims=2)[:, 1], ("lat",), Dict()),
        ("OHT_MEAN",           mean(OHT, dims=2)[:, 1],           ("lat_bnd",), Dict()),
        ("OHT_AM",             OHT_AM,                            ("lat_bnd", "year",), Dict()),
